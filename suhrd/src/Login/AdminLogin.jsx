...
const AdminLogin = () => {
  ...
  const verifyLogin = async (e) => {
    ...
    const url = "http://localhost:4000/api/admin/adminlogin";
    const options = {
      method: "POST",
      headers: {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-purple-500 to-pink-500 p-4">
      <div className="bg-white rounded-2xl shadow-lg w-full max-w-4xl flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="md:w-1/2 bg-white p-8 flex flex-col justify-center">
          <h2 className="text-3xl font-bold text-purple-700 mb-6 text-center">Admin Login</h2>
          ...
          <form className="space-y-4" onSubmit={verifyLogin}>
            ...
            <button onClick={() => navigate('/adminlogin')} type="submit" className="w-full py-2 rounded-md bg-gradient-to-r from-purple-500 to-pink-500 text-white font-semibold" > Login </button>
            <Link to = "/adminsignup"><p>Don't have an account? Sign Up</p></Link>
          </form>
